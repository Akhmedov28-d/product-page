...
const addresSorting = commentData.filter((com) => com.email.endsWith(".com"));

const ID_SORTING = commentData.map((comment) => {
  let newPostId;
  if (comment.id <= 5) {
    newPostId = 2;
  } else {
...
const EMAIl_USER = commentData.map((moll) => moll.email);

const renameArray = emailUser.toString();
console.log(renameArray);

const RENAME_ARRAY = EMAIl_USER.join(', ');
console.log(renameArray);
